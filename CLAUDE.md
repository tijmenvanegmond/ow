...
Every mode is assembled from sources via a manifest into `build/`:
...
- `rules/` — mode-specific rule files consumed by manifests (incl. whole hand-written modes like `brigbonk.ow`)
...
- `build/` — assembled outputs, ready to paste (tracked in git, regenerated by `assemble.js`)
- `docs/` — design/roadmap notes per mode (e.g. `rein-and-weave-1.0.md`)
- `*.owproj` — assembly manifests (repo root)
- `*.settings` (repo root, gitignored) — scratch location for raw lobby pulls; split them into `settings/` + `imports/`/`rules/` and delete
- `errors/` (gitignored) — scratch space for in-game error screenshots
